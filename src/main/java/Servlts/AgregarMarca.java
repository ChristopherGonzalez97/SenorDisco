...
@WebServlet(name = "AgregarMarca", urlPatterns = {"/AgregarMarca"})
public class AgregarMarca extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            int id = em.createNamedQuery("Marca.findAll").getResultList().size()+1;
            Marca marca = new Marca();
            marca.setIdMarca(id);
            marca.setNombre(nombreProducto);
            marca.setImagen(picInBytes);
            em.getTransaction().begin();
            em.persist(marca);
            em.getTransaction().commit();
            HttpSession s = request.getSession();          
            s.setAttribute("MensajePagina", "Marca agregada correctamente <br> redireccionando...");
            s.setAttribute("url", "../Employees/AgregarProducto.jsp");
